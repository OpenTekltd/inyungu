...
// or using: $rawData=User::model()->findAll();
    ...
    if ($payment != 0) {
        ...
        $days = $data['days'];
        $isigayee = (strtotime($deadline) - strtotime(date('Y-m-d'))) / (60 * 60 * 24);
        $isigaye = number_format($isigayee);
        ...
        <div class="span3 ">
            <div class="stat-block">
                <ul>
                    ...
                    <center>
                        <?php if ($isigaye >= 0 && $isigaye < 5) { ?>
                        <span class="blink" style="color: whitesmoke; background-color: red;"><?php echo $isigaye ?></span>
                        <?php } else if ($isigaye > 5) { ?>
                            <span><?php echo $isigaye ?></span>
                        <?php } else { ?>
                            <span class="blink" style="color: whitesmoke; background-color: red;"><?php echo 'Iminsi yarangiye' ?></span>
                        <?php } ?>
                        <span>Iminsi usigaje</span>
                    </center>
                    </li>
                </ul>
            </div>
...
//                    'chart'=> array('type'=>'bar'),
    ...
    <?php
    $num_of_all_voucher = Yii::app()->db->createCommand('SELECT count(*) FROM igu_voucher')->queryScalar();
    $num_of_all_voucher_used = Yii::app()->db->createCommand('SELECT count(*) FROM igu_voucher where status = 2')->queryScalar();
    $num_of_all_voucher_not_used = Yii::app()->db->createCommand('SELECT count(*) FROM igu_voucher where status = 0')->queryScalar();
            ...
            <div class="summary">
                <ul>
                    <li>
                        ...
                        <span class="summary-number"> <?php echo $num_of_all_voucher; ?>
                        </span>
                        <span class="summary-title"> Voucher zihari zose</span>
                    </li>
                    <li>
                        <span class="summary-icon">
                            <img src="<?php echo $baseUrl; ?>/img/folder_page.png" width="36" height="36" alt="Active Members">
                        </span>
                        <span class="summary-number"><?php echo $num_of_all_voucher_used; ?></span>
                        <span class="summary-title"> Voucher zakoreshejwe</span>
                    </li>
                    <li>
                        <span class="summary-icon">
                            <img src="<?php echo $baseUrl; ?>/img/credit.png" width="36" height="36" alt="Monthly Income">
                        </span>
                        <span class="summary-number"><?php echo $num_of_all_voucher_not_used; ?> </span>
                        <span class="summary-title"> Voucher zisigaye</span>
                    </li>
